...
BEGIN
    IF EXISTS (
        ...
        INNER JOIN [AspNetRoles] AS target ON target.[NormalizedName] = UPPER(source.[Name])
        WHERE target.[Id] <> source.[Id])
    BEGIN
        THROW 51002, 'A legacy Domain.Roles row has the same normalized name as an existing AspNetRoles row with a different Id. Resolve the duplicate role mapping before rerunning.', 1;
    END;
