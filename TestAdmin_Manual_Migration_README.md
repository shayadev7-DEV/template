...
- **Legacy role migration is guarded.** If `Domain.UserRoles` contains memberships that cannot be mapped to an Identity user through `AspNetUsers.DomainUserId`, the script throws and stops before dropping the table.
