...
public sealed class AuthenticationService(
    ...
    Microsoft.Extensions.Options.IOptions<AuthenticationOptions> options) : IAuthenticationService
{
    public Task<AuthenticationResult> LoginAsync(LoginDto dto, CancellationToken cancellationToken = default)
    {
        var provider = providers.FirstOrDefault(x =>
                string.Equals(x.Name, options.Value.Provider, StringComparison.OrdinalIgnoreCase))
            ?? throw new BusinessException($"Authentication provider '{options.Value.Provider}' is not registered.");
