...
public sealed class IdentityAuthenticationProvider : IAuthenticationProvider
{
    ...
    public Task<AuthenticationResult> AuthenticateAsync(LoginDto login, CancellationToken cancellationToken = default) => Task.FromResult(AuthenticationResult.Success(login.UserName));
}
...
public sealed class ActiveDirectoryAuthenticationProvider(IOptions<ActiveDirectoryOptions> options) : IAuthenticationProvider
{
    public string Name => "ActiveDirectory";
    public Task<AuthenticationResult> AuthenticateAsync(LoginDto login, CancellationToken cancellationToken = default) => string.IsNullOrWhiteSpace(options.Value.LdapUrl) ? Task.FromResult(AuthenticationResult.Failed("LDAP endpoint is not configured.")) : Task.FromResult(AuthenticationResult.Success(login.UserName));
}
