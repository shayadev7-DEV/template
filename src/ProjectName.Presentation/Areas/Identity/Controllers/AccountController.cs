...
public sealed class AccountController(IAuthenticationService authentication) : Controller
{
    ...
    public async Task<IActionResult> Login(LoginViewModel model, CancellationToken cancellationToken)
    {
        ...
        var result = await authentication.LoginAsync(new LoginDto(model.UserName, model.Password, model.RememberMe), cancellationToken);
        if (!result.Succeeded) { ModelState.AddModelError(string.Empty, result.FailureReason ?? "Login failed."); return View(model); }
